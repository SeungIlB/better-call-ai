...
후속 `importLegalData --args=flow-evaluate`는 가상 확정본을 로컬 DB에 준비하여 실제 발췌·검색·생성 서비스를 함께 실행한다. 새 사례 6개에서 서버 응답은 6/6 통과했지만 목표 조문은 2/4만 검색됐고 시행일을 장래로 잘못 설명한 답변이 있었다. **품질 통과 상태가 아니며 검색 누락·시행일 판단 보완이 다음 우선순위다.** [연결 평가 결과·실행 범위](docs/legal-flow-evaluation.md)를 참고한다. OCR 인식과 HTTP 인증 자체는 이 실제 호출 평가에 포함하지 않는다.
