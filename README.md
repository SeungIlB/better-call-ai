...
- 외부 호출 중 DB 연결과 행 잠금을 유지하지 않는다. 서버 인스턴스당 최대 2건, 파일당 1건을 처리한다. 파일당 기존 20 MiB 제한을 유지하며, 응답 대기는 최대 90초, 연결은 3초, 출력은 최대 16,000토큰·100,000자·응답 본문 1 MiB다. 미완료·거절·비정상 응답은 성공 원문으로 저장하지 않는다. OCR 응답 ID·모델·입출력 토큰은 채팅과 별도로 기록한다. 무료 업로드 횟수 정책을 변경하거나 유료 일일 제한을 추가하지 않는다.
...
다음 단계는 고정 평가 질문을 만들고 일상어→법률 용어 검색 보완, 키워드·벡터 혼합 검색을 비교하는 것이다. 특정 예시에만 맞춘 강제 조문 삽입은 하지 않는다. 부칙의 경과규정과 과거 사건 적용 법령, 판례 데이터, 인용 검증도 별도 작업이다. 기존 채팅에는 아직 RAG 검색을 연결하지 않았으며 사실 정리 전용 안전 지침을 유지한다.
