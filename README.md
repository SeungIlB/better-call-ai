...
- PostgreSQL 16 + pgvector 전체 Flyway migration V001~V015
...
- 업로드 성공은 UPLOADED, 악성코드 검사 상태는 pending이다. 파일 구조 검사는 악성코드 검사가 아니며 OCR·AI에 자동 전달하지 않는다.
...
- 원본 다운로드, 악성코드 검사, OCR, OCR 확정 시 즉시 삭제, 사용자별 일일 업로드 제한과 업로드 idempotency는 후속 작업이다. 인터넷 공개 전 격리 파싱·악성코드 검사와 사용자별 요청/저장량 제한을 보완해야 한다.
