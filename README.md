...
단계별 변경·검증·남은 한계는 [구현 기록](docs/implementation-progress.md)을 확인한다. 전체 구현·검증·잔여 확장 범위는 [프로젝트 완료 리포트](docs/project-completion-report.md)에 누적 기록한다. 최신 사용자 지시에 따라 로컬 커밋만 하고 원격 push는 하지 않는다.
...
입력 변경 중 생성된 결과는 저장하지 않는다. 사건 진술/OCR 확정 변경 시 이전 결과는 `stale=true`로 유지하며 삭제된/타인 사건은 404다. 실행 예약은 5분 만료 후 조회·요청 시 실패로 회수하며 자동 재호출하지 않는다. DB `aiops.analysis_runs`와 확정본 참조를 사용하고, 질문·발췌·출처·생성 결과를 사건 삭제 정책에 따라 저장한다. 기존 `legal-draft` API는 여전히 저장 없는 미리보기다. 이 단계는 법률 판단 승인이나 전체 문서 분석이 아니다.
...
채팅 시스템 프롬프트는 사실 정리, 상충·누락 정보 확인, 자료 준비 안내로 제한한다. 확정 OCR·사진 관찰 발췌는 최대 5개 자료를 검토용 문맥으로 연결하지만 진위·법적 효력을 확정하지 않는다. 법률 검색 출처는 분석 API에서 제공한다. `OPENAI_CHAT_REASONING_EFFORT`(기본 빈 값: 옵션 생략)와 `OPENAI_CHAT_MAX_OUTPUT_TOKENS`(기본 1200, 허용 1200~16000)로 OCR과 별도로 설정한다. 출력 예산에는 추론 토큰도 포함되며 모델별 지원값 확인이 필요하다. [프롬프트·설정·검증 기준](docs/ai-prompt-policy.md)을 참고한다.
...
- 이번 단계는 백엔드 OCR 검토 흐름이다. 원본 미리보기와 자동 개인정보 탐지는 후속 작업이다. 확정 텍스트·사진 관찰은 채팅 문맥에 제한적으로 연결되며, 법률 RAG 출처는 분석 API에서 별도로 제공한다. 클라이언트는 OCR 텍스트를 HTML로 직접 삽입하지 않는다. 테스트는 로컬 HTTP 고정 응답과 PostgreSQL을 사용하며 실제 문서의 인식 정확도는 별도 확인이 필요하다.
...
서버에서 앱·DB·Redis를 함께 배포하려면 [Docker Compose 서버 배포](docs/compose-deployment.md)를 따른다. `.env` 설정 후 `docker compose up -d --build`로 프론트와 API까지 실행한다.
...
로그인 제한은 이메일 단위이며, 모든 `/api/v1` 경로에는 인스턴스별 IP·경로 순간 요청 제한(기본 분당 600회)이 추가로 적용된다. 상품 사용량·업로드 일일 한도와는 별도 보호 장치다.
...
- 신규 업로드는 바이러스 검사 없이 PDF·이미지 형식과 크기·페이지 제한을 확인한다. 성공 시 상태는 UPLOADED다. 공개 환경에서는 신뢰할 수 없는 원본을 다루므로 운영 범위를 별도로 검토한다.
...
- FREE는 UTC 하루 10회(한국 시각 오전 9시 초기화), 유효한 PAID는 일일 횟수·총용량 제한이 없다. 파일당 20MiB·PDF 30쪽·사건당 10개/100MiB 제한은 유지한다.
...
- `Idempotency-Key` UUID 헤더가 필수다. 같은 사용자·키·사건·파일명·정규화 MIME·실제 파일 내용이면 재저장 없이 같은 파일의 현재 메타데이터를 201로 반환한다.
- 다른 내용을 같은 키에 보내면 409 FILE_010, 처리 중이면 409 FILE_011, 무료 횟수 초과는 429 FILE_009다. 키 누락·형식 오류는 400이다. 키는 사용자별로 구분된다.
- 동일 키 실패 재요청은 저장된 오류를 반환하며 저장·차감을 반복하지 않는다. 명시적으로 새 시도를 하려면 새 키를 사용한다. 성공한 파일의 사건이 삭제되면 재요청도 404다.
- 내용 검증 전에 별도 트랜잭션으로 사용량을 예약한다. 예약 이후 내용 오류·경합 실패도 횟수에 포함되며 환급하지 않는다. 기본 입력 오류·한도 초과로 예약하지 못한 요청은 차감하지 않는다.
...
- multipart 수신과 내용 해시 계산은 재요청에도 필요하다. 이번 작업은 저장의 중복 방지이며 네트워크 요청 속도 제한은 별도 작업이다. OCR·채팅의 상품 사용량과 토큰 제한은 아직 연결하지 않았다.
...
응답에는 사건 버전·사용 발췌문, `status`, 사실 요약 `summary`, 최대 3개의 `findings`(조건부 설명·원문 인용·서버가 연결한 공식 출처), 최대 3개의 `questions`, 최대 5개의 `actionDraft`(상대방에게 보낼 사실 중심 초안), 최대 5개의 `checklist`(확인·준비할 행동), 검토 안내 `notice`가 포함된다. 초안과 체크리스트는 사건 기록과 확정 자료에 근거해 생성되며 사용자가 전송·실행 전에 수정·확인한다. 출처 번호와 원문 인용이 일치하지 않거나 응답이 미완료·거부·형식 오류이면 503 `LEGAL_ANSWER_001`로 처리한다. 출처 일치는 법률적 타당성 보장이 아니다.
...
통합·출처 검증은 로컬 응답과 고정 자료로 테스트한다. 후속으로 가상 사례 6개와 수집된 민법 조문 2개를 고정하여 실제 생성 평가를 진행하고 요약·질문 범위 지침을 보완했다. `importLegalData --args=draft-evaluate`로 재실행할 수 있으며 실제 API 비용이 발생한다. 평가 설정은 Sol·none·1200토큰·40초이며 운영 모델을 자동 설정하지 않는다. [평가 결과와 한계](docs/legal-draft-evaluation.md), [프롬프트와 설정 기준](docs/ai-prompt-policy.md)을 참고한다. 검색부터 생성까지의 실제 연동 품질, 법률 전문가 검토, 판례·과거 적용 법령·전체 문서 분석은 이번 평가에 포함하지 않았다.

후속 `importLegalData --args=flow-evaluate`는 가상 확정본을 로컬 DB에 준비하여 실제 발췌·검색·생성 서비스를 함께 실행한다. 새 사례 6개에서 서버 응답은 6/6 통과했지만 목표 조문은 2/4만 검색됐고 시행일을 장래로 잘못 설명한 답변이 있었다. **최초 평가에서는 품질 통과로 판단하지 않았다.** [연결 평가 결과·실행 범위](docs/legal-flow-evaluation.md)를 참고한다. OCR 인식과 HTTP 인증 자체는 이 실제 호출 평가에 포함하지 않는다.

검색 누락 표현과 서버 기준일(`referenceDate`, 한국 날짜)을 보완한 뒤 같은 사례에서 목표 조문 4/4, 서버 검증 6/6을 확인했다. 장래 시행 오판은 이번 응답에서 관찰되지 않았고 기존 검색 질문군의 15/15 적중도 유지했다. 수정에 사용한 사례의 재평가이므로 독립 평가나 법률 정확성 보장이 아니다. 외부 API 계약과 모델·추론 설정은 변경하지 않았다.
