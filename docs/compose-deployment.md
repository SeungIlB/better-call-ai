...
Compose는 앱의 DB·Redis·ClamAV 주소를 각각 `postgres:5432`, `redis:6379`, `clamav:3310`으로 지정한다. `.env`의 localhost 연결 주소는 기존 호스트 `bootRun`용으로 유지할 수 있다. 앱·인증 DB 계정명은 초기화 SQL과 동일한 `legal_ai_app`, `legal_ai_auth`로 고정한다.
...
PostgreSQL·Redis·ClamAV의 헬스 체크가 통과한 뒤 앱이 시작하고 Flyway가 DB 스키마를 적용한다. ClamAV는 이미지에 포함된 `clamdcheck.sh`로 준비 상태를 확인하며 최초 바이러스 DB 다운로드에 시간이 걸릴 수 있다. 시작이 지연되면 `docker compose logs --tail=100 clamav`로 확인한다. 현재 ClamAV 메모리 상한은 4GB이며 앱·DB·빌드에 필요한 자원도 확보해야 한다.
...
PostgreSQL, Redis, ClamAV 데이터와 OCR 확인 대기 중인 임시 원본은 Docker 이름 있는 볼륨에 보관한다. 임시 원본은 컨테이너 재생성 시에도 유지되지만 기존 확정·만료 삭제 정책은 그대로 적용된다. DB·Redis·ClamAV 포트는 호스트의 loopback에만 공개한다.
...
호스트에서 Java 앱을 실행하는 개발 방식은 `docker compose up -d postgres redis clamav` 후 Windows에서 `.\scripts\run-local.ps1`을 사용한다. Compose 앱과 호스트 앱을 같은 포트에 동시에 실행하지 않는다.
