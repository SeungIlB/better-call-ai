...
const domainLabels = {housing_lease:'주택 임대차', vehicle_accident:'차량 사고', assault:'폭행', labor:'노동·임금', consumer:'소비자 분쟁', commercial:'상거래', family:'가사 분쟁', inheritance:'상속', defamation:'명예훼손', personal_injury:'개인 상해'};
const domainRoles = {housing_lease:['임차인','임대인'], vehicle_accident:['운전자','차량 소유자','탑승자','보행자'], assault:['피해 주장자','상대방','목격자'], labor:['근로자','사용자','대리인'], consumer:['소비자','사업자','대리인'], commercial:['거래 당사자','사업자','대리인'], family:['본인','상대방','대리인'], inheritance:['상속인','피상속인','대리인'], defamation:['피해 주장자','게시자','목격자'], personal_injury:['피해 주장자','상대방','목격자']};
...
const footer = '<footer>분쟁 자료를 정리하는 검토용 서비스예요. 법률적 판단이 필요할 때는 전문가와 확인해 주세요.<br>업로드 원본은 임시 보관되며 OCR 확정 후 삭제돼요. 본인이 보유한 원본은 따로 보관해 주세요.</footer>';
...
async function workspace() {
  ...
  shell(`<button class="ghost" data-act="home">← 나의 사건</button><div class="welcome row wrap"><div><div class="eyebrow">CASE WORKSPACE</div><h1>${esc(current.title)}</h1><small>${esc(domainLabels[current.disputeDomain] || current.disputeDomain || '주택 임대차')} · ${esc(current.userPartyRole || '역할 미입력')} · 입력 ${current.version}차 · ${date(current.updatedAt)} 수정</small></div><span class="pill">${esc(labels[current.status] || current.status)}</span></div><nav class="tabs" aria-label="사건 메뉴">${parts.map(([id,title]) => `<button data-act="tab" data-id="${id}" class="${tab === id ? 'active' : ''}" ${tab === id ? 'aria-current="page"' : ''}>${title}</button>`).join('')}</nav><div id="workspace"></div>`, current.title);
  const node = document.querySelector('#workspace');
  if (tab === 'overview') node.innerHTML = `<div class="workspace-grid"><form id="statement-form" class="panel stack"><h2>기록한 상황</h2><label class="field">분쟁 분야<select name="disputeDomain" id="case-domain"><option value="housing_lease" ${current.disputeDomain === 'housing_lease' ? 'selected' : ''}>주택 임대차</option><option value="vehicle_accident" ${current.disputeDomain === 'vehicle_accident' ? 'selected' : ''}>차량 사고</option><option value="assault" ${current.disputeDomain === 'assault' ? 'selected' : ''}>폭행</option><option value="labor" ${current.disputeDomain === 'labor' ? 'selected' : ''}>노동·임금</option><option value="consumer" ${current.disputeDomain === 'consumer' ? 'selected' : ''}>소비자 분쟁</option><option value="commercial" ${current.disputeDomain === 'commercial' ? 'selected' : ''}>상거래</option><option value="family" ${current.disputeDomain === 'family' ? 'selected' : ''}>가사 분쟁</option><option value="inheritance" ${current.disputeDomain === 'inheritance' ? 'selected' : ''}>상속</option><option value="defamation" ${current.disputeDomain === 'defamation' ? 'selected' : ''}>명예훼손</option><option value="personal_injury" ${current.disputeDomain === 'personal_injury' ? 'selected' : ''}>개인 상해</option></select></label>${area('상황 설명','originalStatement',current.originalStatement,'required maxlength="50000"')}<label class="field">나의 역할<select name="userPartyRole">${(domainRoles[current.disputeDomain] || domainRoles.housing_lease).map(role => `<option value="${esc(role)}" ${current.userPartyRole === role ? 'selected' : ''}>${esc(role)}</option>`).join('')}</select></label>${area('원하는 해결','userGoal',current.userGoal || '','maxlength="2000"')}<p>상황·역할·목표를 바꾸면 이전 분석은 최신 아님으로 표시돼요.</p><button>수정 내용 저장</button></form><aside><section class="panel"><h2>원하는 해결</h2><p class="pre">${esc(current.userGoal || '아직 입력하지 않았어요.')}</p><button class="secondary" data-act="tab" data-id="files">자료 확인하기 →</button></section><section class="panel"><h3>이 순서로 준비해요</h3><ol class="steps"><li>알고 있는 상황을 기록해요</li><li>자료의 인식 내용을 확인해요</li><li>근거와 미확인 사실을 살펴봐요</li><li>상대방에게 보낼 문장을 준비해요</li></ol><button class="danger" data-act="delete-case">사건 삭제</button></section></aside></div>`;
...
function renderResult() {
  ...
  node.innerHTML = `<section class="panel"><div class="row wrap"><h2>${a.version}차 검토 결과</h2><span class="pill ${a.stale ? 'warn' : ''}">${a.stale ? '최신 아님' : '검토용 안내'}</span></div>${a.stale ? '<div class="note warn">사건 내용이나 자료가 바뀌었어요. 아래는 이전 입력의 결과예요. 현재 자료로 새 분석을 실행해 주세요.</div>' : ''}<h3>기록된 상황</h3><p class="pre">${esc(r.summary)}</p>${conflicts.length ? `<div class="note warn"><strong>자료 간 확인이 필요한 차이</strong><ul>${conflicts.map(item => `<li>${esc(item)}</li>`).join('')}</ul><small>자동 판정이 아니라 비교 후보예요. 원본과 사실관계를 직접 확인해 주세요.</small></div>` : ''}<h3>사용한 발췌문</h3><details><summary>${r.evidence.start}–${r.evidence.end} / 전체 ${r.evidence.totalLength}자 ${r.evidence.partial ? '· 일부 발췌' : ''}</summary><p class="quote">${esc(r.evidence.text)}</p></details><div class="section-title"><h2>함께 확인할 법률 근거</h2><small>${r.findings.length}개</small></div>${r.findings.map(f => `<article class="finding"><h3>${esc(f.source.heading)}</h3><p>${esc(f.explanation)}</p><blockquote class="quote">${esc(f.quote)}</blockquote>${officialLink(f.source.sourceUrl) ? `<a class="source-link" href="${esc(officialLink(f.source.sourceUrl))}" target="_blank" rel="noopener noreferrer">공식 원문 확인 ↗</a>` : '<small>원문 링크 확인 필요</small>'}<small> · 시행일 ${esc(f.source.effectiveFrom)}</small></article>`).join('') || '<div class="note warn">직접 연결할 법률 근거가 부족해요. 자료와 질문을 보완해 주세요.</div>'}<div class="section-title"><h2>추가로 확인해 주세요</h2></div><ul class="steps">${r.questions.map(q => `<li>${esc(q)}</li>`).join('') || '<li>제시된 추가 질문이 없어요.</li>'}</ul><div class="note">${esc(r.notice)}</div><button data-act="tab" data-id="action" ${a.stale ? 'disabled' : ''}>다음 행동 준비하기 →</button></section>`;
}
function renderAction() {
  ...
  const actionDraft = a?.result?.actionDraft || [];
  const checklist = a?.result?.checklist || [];
  document.querySelector('#workspace').innerHTML = `<div class="workspace-grid"><section class="panel"><h2>상대방에게 보낼 문장 준비</h2><p>AI가 사건 기록과 확인한 근거를 바탕으로 초안을 만들었어요. 사실과 요청을 직접 확인하고 수정해 주세요.</p><form id="message-form" class="stack">${area('요청 메시지 초안','message',savedMessage || actionDraft.join('\n\n') || messageDraft(current.originalStatement,current.userGoal,questions),'class="doc-area" required maxlength="60000"')}<label class="check"><input name="reviewed" type="checkbox" required>내용을 직접 확인했어요. 전송은 제가 별도로 진행할게요.</label><button>텍스트 파일 내려받기</button></form></section><aside class="panel"><h2>다음 행동 체크리스트</h2><div class="stack">${(checklist.length ? checklist : ['원본 자료를 별도로 보관해요.','날짜·금액·상대방에게 알린 내용을 확인해요.','미확인 사실을 단정하지 않아요.','원하는 조치와 협의할 내용을 적어요.']).map(item => `<label class="check"><input type="checkbox">${esc(item)}</label>`).join('')}</div><div class="note">법적 기한이나 권리 유지에 관한 판단이 필요하면 전문 상담으로 확인해 주세요.</div>${questions.length ? `<h3>더 확인할 내용</h3><ul class="steps">${questions.map(q => `<li>${esc(q)}</li>`).join('')}</ul>` : ''}</aside></div>`;
}
async function renderChat() {
  const page = await api(casePath(`/chat/turns?page=${chatPage}&pageSize=20`));
  document.querySelector('#workspace').innerHTML = `<section class="panel"><h2>상황을 더 정리해 볼까요?</h2><p>이 대화에는 사건 초기 진술과 현재 확정한 OCR·사진 관찰이 참고 문맥으로 포함돼요. 법률 근거와 출처는 분석 화면에서 별도로 확인해 주세요.</p><div class="button-row"><button class="secondary" data-act="tab" data-id="analysis">법률 근거 분석하기</button></div>${[...page.items].reverse().map(t => `<article class="chat-turn"><h3>나의 질문</h3><p>${esc(t.question)}</p><h3>정리 도우미</h3><p>${esc(t.answer || '완료된 답변이 아직 없어요.')}</p>${t.retryAllowed ? `<button class="secondary" data-act="retry-chat" data-id="${esc(t.id)}" data-attempt="${t.attempt}">이 질문 재시도</button>` : ''}</article>`).join('') || '<div class="empty"><p>헷갈리는 사실이나 더 정리하고 싶은 내용을 적어 주세요.</p></div>'}${pager('chat',chatPage,page.hasNext)}<form id="chat-form" class="stack">${area('추가로 정리할 내용','content','','required maxlength="4000"')}<button>질문 보내기</button></form></section>`;
}
...
app.addEventListener('click', event => {
  ...
  perform(button, async () => {
    ...
    if (act === 'delete-file' && confirm(`'${button.dataset.name}' 자료를 삭제할까요? 확정 OCR과 분석에서도 제외되고 원본은 즉시 정리돼요.`)) {
      await api(casePath(`/files/${button.dataset.id}`), {method:'DELETE'});
      if (selectedFile?.id === button.dataset.id) { selectedFile = null; ocr = null; clearPreview(); }
      await renderFiles(); toast('자료를 삭제했어요.');
    }
...
app.addEventListener('change', event => { if (event.target.id === 'evidence-select') updateExcerpt(); if (event.target.id === 'case-domain') updateCaseRoles(event.target.value); });
