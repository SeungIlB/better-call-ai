...
function newCase() {
  shell(`<button class="ghost" data-act="home">← 나의 사건</button><div class="welcome"><div class="eyebrow">NEW CASE</div><h1>어떤 일이 있었나요?</h1><p>법률 용어를 몰라도 괜찮아요. 알고 있는 사실부터 적어 주세요.</p></div><form id="case-form" class="panel stack">${field('사건 이름','title','','required maxlength="200" placeholder="예: 월셋집 천장 누수 수리"')}<label class="field">분쟁 분야<select name="disputeDomain" id="case-domain"><option value="housing_lease">주택 임대차</option><option value="vehicle_accident">차량 사고</option><option value="assault">폭행</option></select></label><label class="field">나의 역할<select name="userPartyRole" id="case-role"></select></label>${area('상황 설명','originalStatement','','required maxlength="50000" placeholder="언제, 어디서, 어떤 일이 있었는지 적어 주세요. 모르는 사실은 미확인이라고 남겨도 괜찮아요."')}${area('원하는 해결','userGoal','','maxlength="2000" placeholder="예: 누수 원인을 확인하고 수리 일정을 협의하고 싶어요."')}<div class="button-row"><button>사건 만들기</button><button type="button" class="ghost" data-act="home">취소</button></div></form>`, '새 사건');
