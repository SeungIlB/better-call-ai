...
app.addEventListener('submit', event => {
  ...
  perform(form.querySelector('button[type="submit"],button:not([type])'), async () => {
    ...
    if (form.id === 'ocr-confirm-form') { if (document.querySelector('#ocr-save-form textarea').value !== ocr.latestRevision.correctedText) throw new Error('수정한 내용을 먼저 저장해 주세요.'); await api(filePath('/confirm'),{method:'POST',body:{revisionId:ocr.latestRevision.id,sensitiveDataReviewed:!!data.reviewed}}); clearPreview(); selectedAnalysis = null; await workspace(); toast('수정본을 확정했어요. 분석에 사용할 수 있어요.'); }
