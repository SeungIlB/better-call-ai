...
try {
  ...
  await page.getByLabel('사건 이름').fill('현장사진 실제 연동 검증');
  await page.getByLabel('상황 설명').fill('사용자 설명: 천장에 누수가 있다. 누수 원인, 발생일, 임대차 계약 내용, 상대방 통지 여부와 책임은 미확인이다.');
  await page.getByLabel('원하는 해결',{exact:true}).fill('현재 제공한 사진과 설명만으로 확인 가능한 범위와 추가로 필요한 자료를 정리한다.');
  ...
  await page.getByLabel('인식 내용 수정').waitFor();
  ...
  await page.getByRole('status').filter({hasText:'수정본을 확정했어요'}).waitFor();
  ...
  await page.getByLabel('확인할 질문').fill('사용자 설명으로는 천장에 누수가 있습니다. 확정 OCR에는 글자가 없습니다. 현재 입력만으로 확인된 사실과 확인되지 않은 사실을 구분해 주세요. 사진의 누수 원인이나 책임을 추정하지 마세요.');
